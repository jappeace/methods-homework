...
their gain. If there are more verteci at a particular index then they're
...
array is used for constant speed access when the connections of a vert their
gain needs to be updated after moving it to the other partition.
So you can just unty the ellemnt of the linked list (or if it has no
previous in my case its in the fornt of the bucket and I remove it from
there and reinsert the next element).

After knowing the basic datastructers the implementation is quite trivial
...
to try and implement the Fidducia Mathesis local search heuristic.
I think I've succeeded so my final research question is how does the 
Fidducia Mathesis algorithm compare to first
improvement vertex swap?

As can be seen in the results section the Fidducia Mathesis algorithm is
...
to produce better results, I think this is becuase highest gain is considered
first.

My other research topic is selection pressure, turns out that tournament is 
the best for GLS in case of fidducia mathesis, but it also takes the mot 
amount of time.

\subsection{Statistics}
The error marges in the graph are the maximum range of the results so these
are round numbers when substracted from the low or added to the high. The low
and high values are calulated with something called a confidence interval.
It alligns pretty well with the tTest results so I guess its ok.

\newpage
\section{Results}
Fidducia Mathesis dominates vertex swap first improvement in both runtime and
optima found. Combined with GLS it gets such a good score that I became
skeptical and decided to test the anwser more thoroughly.

Timewise Fidducia Mathesis dominates vertex swap first improvement. This
is because it starts with an incredibally steep hillclimb that flattens
out quite quicly. So you just need to pass over the string 4 or 5
times and then you are at the local optimum. Vertex swap first improvemt
increases linearly however. So it takes a lot more string itterations
and evaluations to get there.

The improvements found by Fidducia Mathesis are also a lot better, especially
...
start at index 0 of the string and take any improvment I can find, this
...
left, you can't really do both. Fidducia Mathesis doesn't suffer from both
...
Something I found interesting is that the Fiduccia Mathesis local search
doesn't seem to respond well to ILS it just stays in the range of the lower
error marging of MLS. To verify this I cranked up the validation number to
100 (ie 100 times no better improvement found the local search end), and
increased the pertubation to 7 (because initially it looked like 1 to 5
where slowly going down).
However when you look to vertex swap first improvement you can see ILS
responding violently. In fact ILS-1 is tTest similiar to GLS:Killparents.

Selection method seems to be statistically irrelevant for the optimum in
Fidducia mathesis, although you pay a much bigger time price with both
tournament and kill parents selection. Tournament also touches the best 
result, but in other runs the other two did this to.
...
\paragraph{Sceptisicm} So these results seem a little to good to be true honestly,
...
There is a good chanche this is a global optimum.
