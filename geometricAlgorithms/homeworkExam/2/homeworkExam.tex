...
\begin{algorithmic}[1]
	\Function{calculateBiggestDisk}{$H$}
		...
		\EndFor
	\EndFunction
	\Function{calculateBiggestDisk}{$h_{bound}$,$H_{prev}$}
		\State
		$d_0 \gets arbitrarlyLargeDisk()$
...
We will prove that a$calculateBiggestDisk(H)$ will run in $O(n)$ expected
time with help of backwards analises.
