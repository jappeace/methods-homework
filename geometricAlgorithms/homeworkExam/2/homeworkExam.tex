...
Note that this is not an expected complexity because the theorom 7.3 puts
a hard upperbound on the amount of neigbours as long as $n\geq3$.
\subsection{Query time}
The original datastructure had an expected query time of the follwoing order:

\[O(log(n))\]

This was given. Now we will analyse if the extension will impact this
perormance. The substructure is a trapezoidal map which has an expected
query time of $O(log(k))$ where $k$ is the amount of neigbours for a
