...
All lines are found because the outer tree just selects based on the y range
as soon as a split node is encountered its starts reporting.
Then the $x$ interval tree will filter out all lines that are not on
the x at that level.
...
\begin{algorithmic}[1]
	\Function{select}{$v$, $x \times [y:y']$}
		...
		\State
		\Return $\Call{selectSide}{v, x\times y, c_{right}}\cup
		\Call{selectSide}{v,x\times y', c_{left}}$
	\EndFunction
	\State
	\Comment{$d_{irection}$ is a function that indicates which side we're reporting}
	\Function{selectSide}{$v$, $x \times y$, $d_{irection}$}
		\If{$isLeaf(v)$}
			\State
			\Return $\emptyset$
		\EndIf
		\If{$y \leq y_{val}(v)$}
		\State
		\Comment{`$opposite$' is a function that flips $d_{irection}$.}
		\State
		\Comment{So if $d_{irection}$ is $c_{right}$, $opposite(d_{irection})$ is $c_{left}$.}
		\State
		\Return $\Call{selectSide}{opposite(d_{irection})(v), x \times y} \cup
		\Call{selectRange}{d_{irection}(v),x}$
		\EndIf
		\State
		\Return $\Call{selectSide}{d_{irection}(v), x \times y}$
	\EndFunction
