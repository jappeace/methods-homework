...
\begin{algorithmic}
	\Function{findxySilhouette}{$v_{start}, v_{current}, S_{result}$}
	\If{$v_{start}$ is equal to $v_{current}$} 
	...
	\Return \Call{findxySilhoeutte}{
		$v_{start}, v_{next}, S_{result} \cup v_{current}$
	}
	\EndFunction
	...
	\Call{findxySilhoeutte}{
		$v_{start}, Origin(Next(IncidentEdge(v_{start}))), Seq(v_{start})$
	}
