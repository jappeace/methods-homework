...
To initialize we consider a function, that receives the first vertex as
...
\begin{algorithmic}[1]
	...
	\State
	\State
	\Comment{So now we have an edge comming out of an origin with a
	maximized x value, however we still have to find the edge that
	goes over the sillhoutte}
	\State
	...
	\EndFunction
	\Function{findFirstSilhoutteEdge}{$\vec{e}_{start}, \vec{e}_{current}, \vec{e}_{best}$}
	...
	\EndIf
	\State
	\Return \Call{findFirstSilhouteEdge}{$\vec{e}_{start}, Next(Twin(\vec{e}_{current})), \vec{e}_{best}$}
	\EndFunction
\end{algorithmic}
...
of the least rotating edge function. This function traverses
