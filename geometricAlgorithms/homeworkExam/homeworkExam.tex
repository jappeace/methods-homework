...
%TODO looks to simple\dots
\begin{algorithmic}
	\Function{findxySilhouette}{startVert, currentVert, result}
	\If{startVert is equal to currentVert} \State \Return result \EndIf
	\State
	$nextVert \gets currentVert.incidentEdge.next.origin$
	\State
	\Return findxySilhoeutte(startVert, nextVert, result.append(nextVert))
	\EndFunction
	\Function{findxySilAlgoirthm}{startVert}
	\State \Return
	\Call{findxySilhoeutte}{startVert, startVert.incidentEdge.next.origin, Seq(startVert)}
	\EndFunction
	\State 
\end{algorithmic}
