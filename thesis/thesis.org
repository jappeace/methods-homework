...
\todo[inline]{ Add An example case section  of different types, how it works in a doctors appointment? For example an INTJ wants the doctor to just get to the point and be out of there wheareas an ENFP wants to share their entire life story? Perhaps we need to make a selection of 3 quite different types, we can refer later to this to as an experiment}
...
default (ie the first $x$ connections),
...
\todo[inline]{They all need an argument to limit processing}

*** Rational
\toReview
In the current design the rational functions apply order to the children of a
current dialogue node.
Then once finished they will call the $next$ function on the most preffered
...
\toReview
To start we will describe $T_e$. It sees the problem to solve as the doctor
appointment itself.
Therefore it will consistently choose speech acts that could help the doctor to
progress the scenario.
So it wants to put the doctor in a position where he has
as few option as possible except to progress the scenario
(according to the meaning graph, doctor can of course say anything).
If it encounteres a child with a goal in it it will give priority to that.
The higher the goal is the more priority.

\toReview
$T_i$ however uses a different strategy. It wants to help the doctor to analyze
the problem according to the doctor's own internal logic framework
and to do this it wants to give as much options as possible to the
doctor.
