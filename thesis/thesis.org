...
Laying the connections out that are necessary is the next step.
...
(in section [[social practice]] its called data gathering).
...
Namely, perlocuationaryValues and goals (see section [[Mapping to process]]).
However to use these we first need to setup the intial believes of the bot.
Which will be discussed next.

*** Believes
\drafting
setting up =believes.yml= can be seen in listing [[code:yaml:sc-believes]].
Actually loading the believes inside a YAML file presents us with a design
conflict.
...
To steer personality in specific we have: perlocationary values, goals,
