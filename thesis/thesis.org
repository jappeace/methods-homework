...
Then the patterndatabase is constructed, not that although the rawsymbols
contain patterns the eventual symbols don't.
This is because we split up patterns and potential utterances into distinct data
structures. Where the patterns just point to the complete symbols.
