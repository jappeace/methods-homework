...
# Jungian functions as an endomorphism:
  # Personality in a dialogue game
...
\todo{I think the idea of utterence should be introduced here since they contain perlocutionary values in the code, this is quite different from the logic we describe now}
\[ D = (a, s, [D])\]
...
3. Only says the first sentence. This is because we don't keep on popping
  sentences from the dialogue tree if the patient actor still is preffered.
...
** Rough planning.
*** Integration tests
**** Make scenario (probably borrow from thesis)
     1. Right now we can't represnt goals we should fix that
     2. Personal values should be specifiable
     3. Values shouldn't be an enum, its just annoying to work with
     4. Perhaps we should tag more values
**** Test the scenario?
...
+ Perhaps if we should keep popping preffered options until we reach an actor
  that is not the agent?
+ If the preffered action is not an agent say nothing?
** Tooling stuff
