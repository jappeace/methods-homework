...
However the current version doesn't track emotions.
...
So what would happen is connections with perlocutionary values could still be
used the same,
but then the model should be extended on what expected effect the value 
has on the bot.
These are then applied and decay rates are used to return to the norm.
With this the only extension connections would need is to guard for being in
certain emotional states, although perlocutionary values could also be used for
this.
For example if the bot were to know certain utterances could change his emotion
to a more desirable state, it would be enough.
In that way we only would need to encode the expected emotional change per
perlocutionary value, and which emotions are desirable to be in, although
personality could play a role in that.
For example a thinking dominated personality would rather be calm, whereas an
intuitive personality would rather be excited.
