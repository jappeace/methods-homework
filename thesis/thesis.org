...
* Implementation
\drafting
+ We start with replacing AIML I guess
+ Then we go trough the script(s) which we showed dignum and how each component is used?
  + So for example have a subsection for personality before etc, and each time we encounter each new show what elements are used?
