...
\[ s \overset{h}{\to} i \].
\todo[inline]{replace s by perlocutionary value which should be encoded in the edges (s is wrong here, its not the meaning that causes percolutionary responses but the edge that are used to get to it)}
...
progress the scenario (according to the symbol graph). \todo{this needs to be described in the graph though, which is currently not the case}
