...
What we can see directly by comparing these is the change in columns,
or swimming lanes as plant-uml calls it.
...
we're not just limited to case based matching in figuring out what the user said.
As was demonstrated by using regular expressions instead of simple AIML
...
\toReview
What we can see is that if the user sais "Hello" then the bot will reply "Hi".
So what we have here is a mapping function from $\sigma \to \sigma$, where $\sigma$ is a
string (see section [[BDI and Jung]]).
...
in which case AIML is no longer used as knowledge representation.

\toReview
We're also only modelling half of the conversation.
...
process (See section [[BDI + Personality]]).
...
#+RESULTS[598d6e1c753297bcb1ed5689c603cae3cc14bf55]: fig:dep:aimlcats
...
Such as the fact you're stuck with AIML based patterns, 
...
We eventually chose for the latter,
...
And the deployment diagram can be seen in figure [[fig:dep:aimlsyms]].
...
#+CAPTION: Deployment diagram of AIML that maps patterns to symbols
#+LABEL: fig:dep:aimlsyms
#+RESULTS[20bc07fb7eb297ea0af12257ea130b0a25180c48]: fig:dep:aimlsyms
...
This example is of course can't be functional,
since the symbol graph hasn't been introduced (see section [[symbol graph]])
to list any available replies to symbols.
However we can at least use these to to create the connection graph from.
...
\toReview
However we can do much more to simplify,
