...
\newlyCleared
In the background section [[Background]] we will start by looking into 
personality theories [[personality theories]] for their advantages and disadvantages.
Then we'll discuss AI literature in [[Artificial intelligence literature]].
...
make such a system in section [[BDI and Jung]].
...
** Artificial intelligence literature
 # How do I measure that the personality created is in fact in compliance with
...
**** IVE
<<Fatima Architecture>>
\dignum{Don't use BDI but IVA as basis. For example the Fatima architecture. This is more aimed at interaction with characters}
\ask{I was asked to use IVE instead of BDI as basis, but within the fatima paper they use BDI right? So I can keep that around?}

\drafting
Interactive virtual environments are populated by synthetic characters used
to guide an application cite:dias2005feeling.
In this Fatima architecture OCC was used to define and track emotions in their
respective agents.
They also defined `personalities'
trough value based limits on emotions,
decay rate variances,
goals,
reaction rules and action tendencies,
personalities were emulated.

\drafting
This Fatima architecture is an extension upon BDI, and has since been extended
to include a motivational system cite:lim2008improving, and has been renamed
to Fatima-PSI.
...
*** Social practices
 <<social practice>>
 \dignum{Introduction: Why social practices? Relevant for giving context to dialouge, and therefore expectations, norms, social effects etc.}
...
/Plan patterns/ are blue prints for the eventual plan. \dignum{No, plan patterns set the boundaries in which plans can be made}
...
[[file:img/uml/sp-activity.png]]
  ...
  \dignum{A plan pattern isn't an abstract workflow! It only gives time order to parts of a dialogue}
  \cleared
...
is used. \dignum{Give examples, and tell what you would do differently}
...
on MBTI cite:cocu2015agents. \dignum{???} \ask{... what means =???= ?}
However communication was never implemented and personality got reduced to
doing a single action. \todo{ I think he means we should delete this}
...
** Campos
\cleared
Campos used the MBTI to create BDI based agents cite:campos_mabs2009. \dignum{Tell more about this, what means fine grained, how does the architecture of campos look?}
...
\todo[inline]{This should be restructured and reread, trying to figure out what goes on from a birds eye view is to difficult}
...
of the chatbot as discussed in section [[BDI and Jung]].
...
[[BDI and Jung]] in a formal language discussed in section [[Logic of BDI]].
...
#  LocalWords:  Rao Bratman's Grice's anamnesi ENFP ISTP toc se IVE
