...
that is much more succinct then AIML.
...
\drafting
To give a good overview of the problems AIML has with modeling the scenario
we can look at listing [[code:aiml-std-cats]] and a component diagram of what this
means in figure [[fig:dep:aimlcats]].
We use patterns to identify user utterances and attach responses to them.
There are several important things of note here.
First of all, we're only modelling half of the conversation.
There is no way for the bot of knowing that a logical response on the bot's
reply of hello, should usually be followed with a question of the user
"how are you?".
