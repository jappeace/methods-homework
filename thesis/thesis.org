...
\toReview
So we decided conclusivly to use YAML for replacing AIML.
Going back to the problem we tried to solve is how to represent these
symbols in YAML:
...
#+CAPTION: Test attempts at what would pass for normal words according to ALICE.
...
We would argue that these are /not/ normal words, therefore ALICE does not
comply to the AIML standard.
...
- symbol: hello_x
  ...
  utterances:
    name: 
      actor: doctor
      symbol: introducion/my_name_is_x
      match: 0 # just to show, obviously default value will be 0
#+END_SRC
