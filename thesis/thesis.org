...
\drafting
  There the implmenetation fase major architectural changes have happened

\drafting
We have thought about making the architecture ready for allowing multiple bots
...
\todo[inline]{Indicate that the AIML can be replaced with the new format}
\cleared
...
<<multiple conversation partners>>
\drafting
This is currently not supported by the architecture because it introduces new
problems to deliberation.
Such as, when in a dialogue tree do we decide which agent speaks.
But more importantly, it introduces new problems to the pattern matching,
since it has to be decided at which agent an utterance has been directed.

\drafting
To add support for this in deliberation,
it would require the use of a more advanced agent selection mechanism in
dialogue tree.
The actor class should also be more specific in this case.
