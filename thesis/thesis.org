...
*** Integration tests
**** Fix scenario issues
     1. Perhaps we should tag more values (wherever its necessary according to scenario)
     2. Change to popping behavior to re-insert a reply utterance and then or do nothing.
        1. This allows even for low-level replies to think about it.
        2. We of course then need to block low level replies based upon actors
     3. Fix the remembered dialogue to store prelocutionary values.
        1. This is requried for Fi
     4. Investigate why ENFP goes into the unexpected path.
     5. Do another test run.
