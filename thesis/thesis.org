...
#+NAME: tab:word-pair-example
 ...
 a scale of 0 to 100. 
 This means the tendency for introversion would be about 30 on a scale of 100,
...
*** Dialogue tree
 ...
 uttered, $s$ the symbol that was uttered.
...
design was that ENFP didn't utter multiple responses (see appendix [[Turn taking
issue]]). This was solved as discussed in section [[Turn taking]], where we reinsert
the dialogue tree as drools processing, however with the condition that the
functions would prefer actor alternation if equal valuation happens (to prevent
infinite loops).
The second major issue was the $T_i$ function not using its default behavior
(see appendix [[TI random choice first]] and [[TI random choice second]]), 
...
After solving the ply depth we did however decide to make the ISTP dialogue
slightly easier for the chatbot because it required a lot of input at the same
time, however this has little to do with personality.
This is discussed in appendix [[Changed istp test slightly]].
