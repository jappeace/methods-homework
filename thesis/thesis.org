...
\toReview
In this subsection we will discuss the main datastructures used to implement
the ideas from section [[BDI and Jung]].
We use class diagrams to acomplish this which are based upon UML cite:fowler2004uml.
Before we do this I would like to point out several things to keep in mind.

\toReview
Firstly, we do not showeverything precisely such as implemented in the code,
...
between those, follwoing the words of Torvalds.

\toReview
Secondly, its better to think of the classes shown here as datastructures,
...
There exists a real seperation like this in the code base, the lower level
components are in the `salve_drools` projects, whereas the higher level
components are in the `salve_personality` package.
The reason for this seperation is that currently, the bot will simply
...
#+RESULTS[f4a0216896afd6a4c60975c59fc4274b019d2d57]: fig:droolclass

Fourthly we often introduce types where primitives could've been used,
for example with the Scenes or Actors, which can be replaced with a Sring in principle.
However we do this only because we want type safety.
...
#+LABEL: fig:jungclass
#+RESULTS[6e958ae8fbd0df7155d49679dfb443b74d79aec3]: fig:jungclass
...
To do this several new data strucures had to be introduced which can be seen
...
Because StringTemplate enforces strict model view seperation,
