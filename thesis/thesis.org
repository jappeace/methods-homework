...
**** Scenes
\toReview
Once the connection database is constructed we can start working on the scene
database.
This is to support scene transitions from the active scenes,
to neigbouring scenes.
What we want is another pattern database,
but this does for the scene key not contain the patterns of the current scene,
but the patterns of symbols of neigbouring scene's it goes to.
So this pattern database will be used as a last effort, if the active scene
doesn't contain the pattern.
...
Note that this will add a before tag to each connection that the from to lists
generate.
