...
\[ s \overset{h}{\to} i \].
\todo{replace s by perlocutionary value which should be encoded in the edges (s is wrong here, its not the meaning that causes percolutionary responses but the edge that are used to get to it)}
