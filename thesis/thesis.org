...
result, alternativly this is similar to category theory]
...
In this case the /next/ function can play an advisory role to $f_a$.
As unit a function that does not modify behavior and selects a random action can
be chosen, or just the action that results in a message: "I don't know".
...
Because its the least preferred function we just use unit.
