...
#+LATEX_HEADER: \newcommand{\cleared}{\todo[noline, backgroundcolor=white, bordercolor=red]{Newly cleared}}
...
\todo[inline]{Describe Plan patterns are declarative not imperative}
\todo[inline]{They describe just play an supporting role to possible implementaions.}
\todo[inline]{A plan pattern is a way to describe many possible plans, so its a plan prototype}
\todo[inline]{The old one didn't work because there would be examples where it wasn't possible to comply (for example if you have a cold)}
...
#+RESULTS[0a6819d9f31cce0a72033a77e9723a6a659dcd51]: fig:components
...
\todo[inline]{Doctor could prescribe different treatments and or medicine because patients give different inforamtion based on the information they shared.}
\todo[inline]{This would signify the importance of personality within the game}
\todo[inline]{Patient may acutually have a damaged back, and for example the INTJ is there just to score medicine, whereas the ENFP will tell it by accident and get a scan but the ISTP will just start talking about how he worked in construction or something}
\todo[inline]{Don't know how it would affect the ISTP}
\toReview
With the personality theories, AI literature and the serious game discussed
thoroughly, we can make an expectation of results.
We want to discuss what would happen if we would give the chat-bot one
personality over the other.

\toReview
To analyze this we will make a small case study of several different typed
people going to the doctor.
First we have Sander the INTJ, secondly Susie the ENFP and Chris the ISTP.
This type selection will give a rough overview of most jungian functions.
...
*** Sander the INTJ
\toReview
First we should note the dominant and auxiliary functions of the INTJ mbti
type.
An INTJ has as dominant function introverted intuition $N_i$ and as auxiliary
thinking extroverted $T_e$.
We would expect these function to be most dominant in the dialogue.
...
himself probably already had drawn. We could even expect him to ask for the
...
| Docotor | Haha, no, perhaps you just lifted the watering can wrong. |
| Doctor  | I'll subscribe you some pain killers                      |
| /Susie/ | /Thanks doctor, that surely will help!/                   |
...
| Doctor  | I'll give you some pain killers                   |
| /Chris/ | /Thanks doctor/                                   |
...
We want to do something similar but using Jungian function attitudes rather than
the measured MBTI dimensions.
...
\cleared
We will reason about the function attitudes by considering the
type signature of a pure function that can represent them.
A Haskell like syntax is used for this.
We want to have a type signature that can describe the domain, but nothing more.
...
this is of course not desirable (or computationally possible, unless a lazy
language is used, but we don't have that).
Finally We will also look at how to convert user utterances to types we can do
reasoning with.
