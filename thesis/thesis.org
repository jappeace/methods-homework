...
*** Destruction
\drafting
Make sure to kill the kie thread.
Close the websocket.
GC does the rest
...
\drafting
Let the bot support multiple conversation partners (including other bots)
This would require the use of a more advanced agent selection mechanism in
dialogue tree. THe actor class should also be more specific in this case.
Connections could for example be restricted to coalitions rather than just
single actors.

\drafting
better support for if multiple symbols are matched.
Currently just the first one is used.
