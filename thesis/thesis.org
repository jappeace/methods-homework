...
progress the scenario (according to the symbol graph).
So it wants to put the partner in a position where he has
almost no other options except to progress the scenario
(according to the meaning graph, the partner can of course say anything).
...
\todo{Perhaps I can merge this into implementation}
\toReview
