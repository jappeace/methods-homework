...
#+RESULTS[989ab68b4d78c83d14903b2658462d71009d62db]: fig:sp-activity
...
this is of course not desirable (or computationally possible).
...
Where it will block until a $t$ has passed or a new perception $P$ comes in from
the environment.]
...
#+RESULTS[546fbd1c8989e5cc1c146b7dda332099a6ea8e5a]: fig:dialoguetree
...
node?
Same question for rational functions should it sort everything or just the
children list one layer above the leaf nodes.
In our implementation we will go for the approach where irrational functions will
go down the left (most preffered) path to a leaf node and then generate more
meanings, but rational functions will only sort the one layer above the leaf
layer. However there may be exceptions
for example when a rational function is primary.
...
/rational/ functions change the order of possible replies, one layer above the leaf layer,
/irrational/ increase the number of children at the leaf layer.
Both will go to the most preffered option (left most).
...
The meaning graph $G$ is a set of connections $(A, s_1, s_2)$
where $s_1, s_2 \in \mathcal{S} \wedge s_1 \neq s_2$ and $A \subseteq \Lambda$ is the set of
...
allegorically we considered them in the following way.
...
\drafting
Algorithmically this would translate into $S$ going several plies deep into the
...
guarantee that  the $next$ function will bring us to the goal.

\drafting
Now to consider the function attitudes rather than the plain functions. \todo{relevant??}
This is surprisingly difficult.
Existing literature either sticks to the previous step of just defining
$S$ vs $N$,
it doesn't consider function attitudes in interplay with other
functions (when to call next) or it uses terms which are just hard to get a
grip on (Jung himself used primordial images extensively
cite:hall1973primer104nitype for example).
So here be dragons.
...
\toReview
Both $N$ use a depth first approach to try and go to the goal. However $N_i$
starts at the goal and moves towards the matched meaning point,
whereas $N_e$ starts at the matched meaning point and goes towards the goal.
the /next/ function is used to determine which direction to go.
...
meaning and then applies the /next/ function on it.
The $S_i$ however is more conservative and will only pop 2 random meanings by
default (ie the first 2 connections),
however it will construct its own connections of whatever the user said in
response to the bot. Whenever these connections are available they will be
used instead of the random 2 or even more whenever it is available.

\todo[inline]{They all need an agrument to limit processing}
*** Rational
\toReview
First of all note that although we have a game tree,
...
call the /next/ function to do deliberation on the most preffered item.

\toReview
The thinking functions $T$ want to go to the goal as soon as possible without
paying any attention to perlocutionary speech acts.
To do this it will sort the dialogue children according to as much
unique symbols as possible.
If it encounteres a child with a goal in it it will give priority to that.
The higher the goal is the more priority.

\drafting
The difference between $T_e$ and $T_i$ is... \todo{Yes what is the difference}
\todo[inline]{I don't know, perhaps Ti uses memory or something?}
\todo[inline]{What about Te tries to figure out what the goal is of the doctor and tries to work against that whereas ti just tires to goe its own goal??}
\todo[inline]{Or perhaps do the graph drawing thingy for Te and the current T thingy for Ti, problem is that the graph drawing has little to do with Te}

\toReview
Both feeling functions $F$ use the perlocutionary acts to order the children.
$F_i$ uses a predefined value set.
This valuation is done by a lookup table on all avalaible perlocutionary speech
acts.
However $F_e$ tries to figure out what the conversation partners values by
...
Therefore $B = (m, h, h', [m], [D], G)$ \todo{This is just wrong right now (the stuff below needs to be integrated)}
...
| $N_e$    | Meaning graph         |
| $N_i$    | Meaning graph         |
\todo{Fix this table too, its kind off wrong right now}
...
\todo{find a good location to put this chapter/section}
\drafting
...
#+RESULTS[bfb3eb88cfd8095269348f4fa1677f4142a78da1]: fig:architecture-concept
...
This is messy because now we have data, that is supposed to live in xml files,
encoded into our drool files.
...
with a structural scheme. \todo{I find it rather hard to believe I'm the first one to think of this}
...
\drafting
The communicate game was created to make students able to practice \todo{better intro}
