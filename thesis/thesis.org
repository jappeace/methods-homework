#+TITLE: Jungian personality in a chat bot
#+SUBTITLE: A more elegant and flexible method of modeling
...
We chose to use classes wherever possible becuase it provides type safety.
This is only possible if the keys are known before hand.
Which its not in the template system for example, but in most othe cases it is.

\toReview
In listing [[code:java:rawsym]] the entire field sturcture for symbols is shown.
So the fields defined there can be used as keys in symbol files.
...
\toReview
In listing [[code:java:yamlread]] we show how one can load a yaml structure with a
raw class. The respective file is simply passed into a specialized function
shown in listing [[code:java:yamlparse]].
Do note that we can add additional constraints on the model as shown in
listing [[code:java:yamlread]], YAML supports multiple objects being created
from a single file,
however in case of symbols this isn't desirable because
filename plays a role in enforcing uniqueness.

#+CAPTION: Reading with the class from listing [[code:java:rawsym]] and parsing with listing [[code:java:yamlparse]]
...
\toReview
In listing [[code:java:yamlparse]], we show the entire parse code used by our project.
This is in stark contrast to the ALICE bot which was used previously,
which had an entire XML parser intertwined with other logic.
So what is done in that listing is we read the file content wrapped in an input
stream reader, then we use the YAML libraries' =YamlReader= to create a reader,
which parses the file content into object of the type stored in variable =clas=.
Which in case of listing [[code:java:yamlread]] the =RawSymbol= class is.
The function shown in listing [[code:java:yamlparse]] can be used to load any YAML
file with its =Raw*= class.

#+CAPTION: Usinga a library to parse YAML
#+NAME: code:java:yamlparse
#+BEGIN_SRC java
...
As we will see it will create an interesting constraint on connections.
...
and it had a lot more to deal with just getting an environment ready,
...
What our modeling system in YAML asks goes even a step further,
most utterances all actors can make in a particular niche.
We will discuss how we modeled the dialogue and
what methods we used to steer particular personalities over particular paths.

*** Dialogue to YAML
\toReview
In this section we will discuss how we modeled the scenario discussed in
...
\toReview
Our strategy for modeling these selecting a particular dialogue and then
deciding per utterance whether they require a unique symbol, or can be merged
in an existing one, or already have one.
For example we merge the greetings according to listing [[code:yaml:greeting]].
...
\toReview
While putting utterances into symbols, we started thinking about how to group
them in scenes.
...
[[social practice]] as guiding mechanism.

\drafting
The final step is modeling the connections.
Laying the connections out that are necessary is one of the more easy steps.
We just look at the dialogue and link up the utterances as can be seen in
listing [[code:yaml:model:initial-connections]].
Since we stick to the social practice for organazing the scenes we put the
symbol ask_reason_here in the information_gathering scene (its 
...
*** Believes
\drafting
setting up =believes.yml= can be seen in listing [[code:sc-believes.yml]].
