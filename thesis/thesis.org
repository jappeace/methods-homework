...
type.
Before its even inserted into the drools,
...
to do the next step which is parsing this string with pattern matching.
To do this we use the pattern databases from figure [[fig:dbclass]], 
on which we use the active scene as a key and then just match against all,
this results in the =ParsedUtterance= type which can be seen
in figure [[fig:supportclassjava]].
This type then gets again inserted.

\toReview
As a first priority the low level reply rules can be fired.
What they do when fired is removing the =ParsedUtterance= type,
...
which is true in practice, but no literal if else structure is used.
...
\toReview
So the high level processing executes if there is still a  =ParsedUtterance= type
available.
With this we create the initial =DialogueTree=, and remove the =Believes=
from the fact base and put these believes into a =PersonalityProcess= which can
be seen in figure [[fig:supportclassperson]].
The reason why we remove the Believes base is to prevent concurrent modifications.
...
\toReview
After there are no more functions in the list anymore, we know we are done.
And move to the next step where we get the reply from the DialogueTree.
...
If we do send a reply we reinsert the selected =Utterance= as a =PreProcessed=
type.
...
\todo[inline]{So I guess this is besides other things discovered during implementation such as replacing xml. I think the stuff about immutability etc can be deleted because its not very relevant to the thesis, it just solves practical problems}
...
** Immutablity
\todo{Delte this because not relevant for thesis?}
