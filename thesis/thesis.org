...
To accomplish this share goal the following informal rules are used:
/Quantity/ has to do with the amount of information transferred in a single
utterance. When going to the barber and the barber asks
"How do you want your haircut?". A quantitative acceptable answer would be
"Short but with a dark color", non acceptable would be:
"Yes, you know, I used to like long hair, 
but now a days I started to doubt this position. 
So I guess I will follow the tradition of my ancestors and wear my hair short. 
But the color, you see, color is mostly determined by the fracturing of light. 
Therefore I would like to absorb as much as possible to feel more energized by 
the available light. 
Therefore I came to the conclusion to have a short dark haircut".
Giving to little information could also be a problem.
For example replying with "dark color" isn't enough for the barber to do his
job.
He needs to additional inquiry to make sure you just want to have your hair
colored.
/Quality/ is the assumption where people will mostly say things they believe to
be true.
...
Dialogue is cooperative rather than competitive.
So doing an algorithm such as mini-max is out of the question.

\todo[inline]{Everywhere where a how like tag is added should at least have a reference to somewhere else that explains it (or inline explenation although since this chapter tries to remain abstract perhaps thats not always a good idea)}
\drafting
So $T_e$ uses just one ply in the meaning tree and then applies a static
evaluation \todo{How? Perhaps we can use graph drawing of the entire meaning structure and then perform a "as the crow flies" distance check of the goal}
to determine if uttering a certain meaning will get it closer
...
\drafting
Both $F$ functions need a valuation function $h$: \todo{How is this represented in data? Perhaps this can be done on the relation between meanings}
\[ s \overset{h}{\to} i \]
where $i \in \mathbb{N}$.
...
\drafting
N uses a meaning graph where $N_e$ goes breath first and $N_i$ goes depth first. \todo{No}
...
current meaning.
S_e uses sense information, just some meanings that are in the environment. \todo{Also noe}
