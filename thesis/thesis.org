...
\todo{this is just wrong, we just look at the scenes and see if there are anoyne that change scene if no goals are available}
If it encounteres a child node with a goal $\phi$ in it it will give priority to that.
