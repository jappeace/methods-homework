...
Therefore both methods will be supported [fn:: what we really do is just
replace the stars in pattern fields by (.*),
which is a regex group that matches anything,
this does not comply with the experimental results of the ALICE bot,
however since the AIML standard remains vague about that we can assume they
actually just meant anything].
...
and add them within the symbol files like so:
...
\toReview
However we quickly decided against such an approach in fovour of grouping
...
They appear in the follwoing example:
...
before, with two required fields, `who' and `said', which indicates the actor
...
unless stated otherwise, such as:
...
limitation of disalowwing enclosment of variables that are not final:
...
So if you don't do this you can do the following thing:
...
\drafting
This is of course unexpected, and even worse it will make symbol thread unsafe,
there is no way for the symbol class to gaurantee the literal list isn't used
because its owned outside of its scope.
However if we make a shallow copy of the collection the mutation doesn't occur.
