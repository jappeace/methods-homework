...
2apl, jade cite:braubach2003jadex.
 \todo[inline]{These are important frameworks for if you have to plan, and replan in a larger context, however our bot doesn't have this (so we should delete this subsection I guess)}
...
**** Drools
 \todo[inline]{Drools and 2apl are significantly different, so they shouldn't be taken together or compared, we still should describe drools more torougly though}
** The serious game
...
\todo[inline]{This chapter isn't interesting, because it doesn't add anything that is specifically used for this thesis, so it can be removed}
...
\todo[inline]{This can be removed because it isn't used in any java specific code (inside the thesis)}
...
*** Operation
 \drafting
 To understand how the operation of a bot works we can look at it best from the
 point when a message is received and walk trough the steps it takes.
 An overview of this process is giving in the figure [[fig:insert_meaning]].
...
* In conclusion
\todo[inilne]{We need to explicitly say here that Mapping function to attidues to a process (3.4) is valid for the test use cases shown in [[Test Results]]. Although the validity is in no way proven (we just have /a/ instance that is correct, which is different from saying that all instances are correct)}
