...
For example Te has as default behavior, connections that transition to
another scene, whereas Ti prefers connections that lead to more options
(see section [[Thinking]]).
...
however to prevent an infinite loop we made sure the doctor can only answer from
there.

\drafting
So if the status symbol is liked more by feeling function Fi,
why did in testing section [[Testing]] Susie (with dominant Fi) utter a greeting
first?
The answer is low level replies (see section [[Operation]]),
of which an example can be seen in listing [[code:drool:lowlevelreply]].
What this does is execute code upon insertion of a symbol, in this case it just
returns a reply which is a greeting.
...
\drafting
If we also take [[code:yaml:model:gathering]] into consideration,
we can see how thinking functions may like the =ask_reason_here= option,
