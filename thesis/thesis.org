...
\toReview
In listing [[code:java:yamlparse]], we show all the parse code used by the implementation.
We read the file content wrapped in an input stream reader,
then we use the YAML libraries' =YamlReader= to create a reader,
...
This is the =RawSymbol= class in case of listing [[code:java:yamlread]].

#+CAPTION: Using a library to parse YAML
