...
listing [[code:yaml:model:initial-connections]],
we can already see how the functions maybe influenced by this,
we expect for example personalities with a dominant feeling functions to go for
...
by the doctor, simply because it would be strange to continue talking after
asking "How are you?".

\toReview
So if the status symbol is liked more by feeling function Fi,
why did in section [[Testing]] Susie (with dominant Fi) utter a greeting
first?
The answer is in low level replies (see section [[Operation]]),
...
telling about the backpain is a goal in the believes.
...
although they have influence on the amount of options.
...
* In conclusion
\toReview
In this work we have shown a possible interpertation of carl Jung's theory of
types in a chat bot.
We built on top of the work of an existing chat bot,
but decided to replace the core moddeling langauge with an alternative
because there was no space to do deliberation in.
This effort resulted in drools becoming the center of the new chatbot,
where each step of processing a user message was decided by buisness rules.
\todo{This alone opens up a legion of possiblities, however we want the conclusion to be short so discuss it in future work I guess}
...
A first step towards that would be to model the thoeries in section
[[BDI and Jung]] in a formal langauge discussed in section [[Logic of BDI]].
...
** Discussion
\toReview
There are some odds and ends that haven't been adressed.
...
However upon closer inspection of the theory it, turned out to be a rotating order,
along the lines of $ABBA$.
Although this has little effect upon the dominant and auxilary functions,
...
Additional experimentation is required to identify this problem,
...
No actual minimax algorthim is used (or something similiar that would work with
non zero sum games).
