...
\todo{Should I add functions such as goal comparison in here?}
...
#+BEGIN_SRC plantuml :cache yes :file img/uml/jungclass.png :exports results
  ...
  class Goal{
    +utterence:Utterence
    +utility:int
    +isGoal(utterence:Utterence):boolean
  }
  Goal --* Utterance
  class PatternSymbol{
     +pattern:Pattern
...
#+LABEL: fig:jungclass
#+RESULTS[0a2997f9591ea5ff0b150aa0002e07d96e9be563]: fig:jungclass
...
#+NAME: fig:newuttProcess
#+BEGIN_SRC plantuml :cache yes :file img/uml/newuttProcess.png :exports results
  |WebSocket|
  start
  :Utterance received;
  :call chat;
  :Insert as UnparsedUtterance;
  |#AntiqueWhite|Drool|
  :parse message with patterndb into ParsedUtterence;
  :record into believes;
  if(ParsedUtterence) then (is empty)
  :Give default reply;
  |WebSocket|
  ...
  :Remove parsed utt insert first symbol;
  :Symbol into Utterence;
  |#CCDDFF|Personality|
  :Utterence into personality process;
...
#+END_SRC
#+CAPTION: Activity diagram of user utterance processing
#+LABEL: fig:newuttProcess
#+ATTR_LATEX: :width 1.0\textwidth
#+RESULTS[0f1df951c3c44a16e61dfda3fa9e6c50bc9df3fe]: fig:newuttProcess
[[file:img/uml/newuttProcess.png]]
...
+ Message processing occurs at either both agents (they need to record history anyway)
