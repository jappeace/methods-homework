...
\toReview
In this thesis we explore how to add jungian personality to a chatbot.
An existing serious game was taken over, and closely inspect.
A strategy was devised for adding personality while leaving as much of the
orignal architecture in tact.
In this process we replaced the existing AIML based scheme with a novel approach.
This made a RETE engine the center of the chatbot,
and opened up space to have varied responses to a similar utterance
depending on the personality,
while making the modelling langauge less verbose and more precise.
We conclude by demonstarting that in fact this scheme works as expected,
and sum a whole list of possible extensions including a possible future
multilogue system.

#+END_CENTER

\toReview
chatbot, Jung, drools, YAML, AIML
...
**** MBTI
 ...
 It uses a structured approach rather than Carl Jungs projective approach.
 ...
 interpretation of MBTI which is based on Kersey's book "Please understand me",
 ...
 But it has gotten some heavy criticism in from the field of psychology.
...
**** SL-TDI
 ...
 measurement of SL-TDI since the question directly measure the jungian
 functions.
...
First of all there is the data model, which are in principle just java classes.
This data model is called the fact base.
Then we have the rule queries, or left hand side.
These indicate when a rule should be executed.
Finally there is the right hand side, which is just a piece of java code
that can modify the facts, or interlope with outside java code trough so called
global variables.
...
Although we make actors explicit not because of wanting to implement a
multilogue system,
...
\newlyCleared
If the status symbol is liked more by feeling function Fi,
why did in section [[Testing]] Susie (with dominant Fi) utter a greeting
first?
...
but we expect there will be many niches we haven't even considered.
...
So that bot to bot conversations could also use the templated symbols.
...
completely. Although currently this is also a potential problem.
...
Also note that at matching personality could play a role.
