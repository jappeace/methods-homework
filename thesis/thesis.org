#+TITLE: Composeable personality pieces 
...
\ask{So I think the work became agentlike (-ish?) so I should expend this?}
*** Social practices
 <<social practice>>
...
\ask{Delete this? because its kindoff pre-knowledge or redundant?}
\todo[inline]{Isn't this a concern for implmentation? Or find at least some sources}
...
[fn::Strictly this isn't true, but in the formal description I use sets anyway],
or even a type as used in programming languages such as Java.
If you declare a function for example in Java as can be seen in
listing [[code:java-a-function]].

#+CAPTION: Java function
#+NAME: code:java-a-function
#+BEGIN_SRC java
  class A{
...
Note that we define the class A, simply because we have to in Java.
...
Secondly a more precise type allows a /pure function/ to do fewer different
things with the type.
Say you have the following pure function shown in listing [[code:java-a-pure-function]].

#+CAPTION: Java function, which we pretend is pure
#+NAME: code:java-a-pure-function
#+BEGIN_SRC java
  class A{
...
\newlyCleared
How many possible different ints can it return?
The answer is two.
Note that the function can't just create a Random object to generate more
entropy since we said it was pure.
Java can't enforce this, but a language like Haskell does.
...
argument of an function can be a function too.
This is how we combine Jungian function with each other to create personalities
in section [[Composing types]].
This has can also be done in Java as can be seen in listing [[code:java-lambdas]].

#+CAPTION: Lambas
#+NAME: code:java-lambdas
#+BEGIN_SRC java
  interface B{
      int func(boolean a);
  }
...
The only way to get around this knowledge restriction is trough casting,
but that is just a way of ignoring the type system cite:whyavoidcasts.
This is not interesting to explore further because we are making a design based
upon the type system.

\newlyCleared
Then finally the last technique we use (implicitly) is a process called partial
application.
In which we fill up some arguments of a function to create another function.
This is why the /next/ arguments in section [[Composing types]] can be other
function attitudes,
once the next argument is partially applied,
the type of the function changes to that of /next/,
so it can be inserted in another function attitude.
Now you have a chicken and egg problem which is resolved by using an identity
function, or unit. This function does nothing.
The Java analogy can be seen in listing [[code:java-composition]].

#+CAPTION: Java composition
#+NAME: code:java-composition
#+BEGIN_SRC java
  interface B{
      int func(boolean a);
  }
...
at time $t$.
$\mathcal{A}$ denotes the set of all possible actions with $A_t$ with \todo{wut??}
...
Therefore we introduce the set of all possible speech acts as $P$ of which 
a set of speech acts is $P \subseteq \Pi$.
With this we can define utterance $u$ as a tuple:
\[ u = (P,a,s,t) \]
Where $P$ is the set of perlocutionary values uttered, $a$ is the agent that
...
\ask{So since we have integration tests now this chapter is valid?}
...
$P$ is the perlocutionary value set of the speech act
(for example angering or scaring),
...
\[ \pi \overset{h}{\to} i \].
Where $\pi \in \Pi$ is a perlocutionary value.
...
*** Symbol overview
\drafting
For reference a symbol table of all introduced symbols is shown in table [[tab:symbols]].
...
| $\mathcal{P}$ |                             | Set of all possible sense information  | [fn:not-used:Not used in implementation] |
| $P_t$         | $P_t \subseteq \mathcal{P}$ | Perception information $P$ at time $t$ | [fn:not-used] |
| $\mathcal{A}$ |                             | Set of all possible actions | [fn:not-used] |
| $P_t$         | $P_t \subseteq \mathcal{P}$ | Perception information $P$ at time $t$ | [fn:not-used] |
...
\todo[inline]{We should say which classes are which symbols in BDI+Jung per diagram}
...
because that would clutter the screen to much.
...
class diagrams, also to save space.

**** Low level diagram

\toReview
In figure [[fig:droolclass]] the diagram containing the low level data structures
used. These are the basic asssumptions, or building blocks the implementation
is constructed from.
...
utterance. Except for the instance field, they are the same
...
A connection entails a posibility of an utterence, but it does /not/ mean it
will be uttered. Whereas an utterance is a used connection, that posibility
became a realization.
Therefore, we were right to distinct them at type level.

\toReview
These kindoff considerations become especially real when structures are
...
We would say yes because they entail completely different semantics,
the Scene class is used to group symbols and patterns,
whearas the actor class is used to identify actors.
...
reduction.
It would also open up to positibilty to use the implicit covarient relationship,
functions that accept an ANamed argument for example.
As soon as client code uses that, the single inheritance 'slot' Java provides
is occupied forever, or at least untill a major refactor occurs.

**** Believes and dialoguetree
\toReview
In figure [[fig:jungclass]] we can see the higher level structures of Believes and
DialgoueTree. Note that we significantly simplified all lower level components
...
is indeed the believes and after that the DialogueTree.
Believes provide the JungFunctions with well bounded information about the mind
...
don't get all information (the when clause).
...
\toReview
The DialogueTree structure however is a whole other beast then the believes
structure.
...
\toReview
PatternDatabase can store patterns based upon a Scene. There are actually two
constructed of these, the first one constructs the patterns that are in a scene,
...
\toReview
The connection database is a database that stores a connection set from a
symbol. 
...
#+RESULTS[6fc2caef96dffeadc45479279efaa48ded915bd3]: fig:templateclass
...
of classes it introduced and touched.
...
\drafting
So the before class is self recursive, as we've seen several times by now,
however it just has an optional self recursive relationship.
...
\todo[inline]{Go over each class explain what they do?}


# make sure the figures don't spill over into next section
\newpage
*** Initialization
\toReview
...
#+CAPTION: Activity diagram of a server game construction
#+LABEL: fig:initjung
#+RESULTS[3aedd73e556c4aa3c802b3c10a249e7b1fae54ea]: fig:initjung
...
+ No way of giving drools space to do deliberation asside from updating the scenario. 
...
\toReview
So what we mean by human and program readable is that with relativly little
...
Alternativly unlike a human document, such as a book,
...
while its relativly easy for a program to notice this is erronious.
...
pattern can be used.
Therefore in our YAML semantics, they are an additional restiriction on connections.
...
to neigbouring scenes.
The initial idea was to make a simple scene graph that would allow scenes to
look into neigbouring scenes if they couldn't match an utterence string with
their own patterns.
...
instead it contains the patterns of symbols it can go to from neigbouring
scenes.
...
\drafting
We want to say a few words about the programming paradigm used \todo{Do we? Really? Perhaps move this to the appendix}
in the realization of the ideas discussed in this thesis.

*** Java
\todo[inline]{IDK add references or decide to keep this? Perhaps ask dignum?}
\drafting
First note that although the project was written in Java,
a langauge which strongly was in favor of the object oriented programming
paradigm, we wrote most of the code in a functional paradigm style.
What we mean by this is that we preffer logical substition over in place writes.

\drafting
Java makes this relativly hard to do. It has a `final` keyword,
which prevents assignment, but this keyword does noet enforce writing into
underlying data structure.
The awkardness of this keyword is best demonstrated by hacking the lambda
limitation of disalowwing enclosment of variables that are not final in lambdas.
Which can be seen in listing [[code:java-lambda-hack]].

#+CAPTION: Hack around Java's final variable restriction in lambdas
#+NAME: code:java-lambda-hack
#+BEGIN_SRC java
	class LamdaHack<T>{
		T value;
	}
...
\drafting
So one can only have immutable structures by making data strcutures enterly
consist either of values, or other immutable strctures.
An example of this is shown in listing [[code:java-immutable]].

#+CAPTION: An immutable structure
#+NAME: code:java-immutable
#+BEGIN_SRC java
@Immutable
public class Symbol {
	public final String name; // strings are in most languages immutable
	public final Scene scene; // immutable too

	private final List<String> literals; // mutable but private and owned by us
	private final int hash_value; // immutable but no reason to make public
	public Symbol(String name, List<String> literals, Scene scene) {
		this.name = name;
		this.literals = copy(literals); // we must copy to gaurantee immutabilty
	...
	public final List<String> getLiterals() {
		return copy(literals); // we must pass a copy to gaurantee immutabilty
	}
	private static <E> List<E> copy(List<E> from){
...
\drafting
Note that because of the fact that Java collections themselves are mutable we
have to make them private and to access it we need to either make a copy or
provide a stream (a stream is readonly in Java).
Also note that upon assignment we directly make a copy of the litteral list,
because we don't know where that refference has come from and if the
reference has been unnasigned.
So if you don't do this you can break immutablity as is shown by
listing [[code:java-must-copy]].

#+CAPTION: Why we must copy mutable lists
#+NAME: code:java-must-copy
#+BEGIN_SRC java
List<String> literals = Lists.newArrayList("one", "two")
Symbol symbol = new Symbol("unimportant", literals, new Scene("unimportant"));
literals.add("Not so immutable now are you?");
#+END_SRC

\drafting
This is of course unexpected,
and even worse it will make the object symbol thread unsafe,
there is no way for the symbol class to gaurantee the literal list isn't used
because its owned outside of its scope.
Even if you put the synchronized keyword on the get method,
you don't get thread safety.
Because the symbol doesn't own the list.
The only way to gaurantee ownership is by making a copy.
If we make a shallow copy of the collection the mutation doesn't occur as shown
in [[code:java-must-copy]].
...
** Immutablity
\todo[inline]{IDK find a place for this}
...
they return a new modified instance however rather than changing the object in
place.
...
\newpage
* Building
\todo[inline]{Document about python scripts and shell scripts (how to use), make sure succerrer can take it on easily}
<<building>>
\cleared
...
#  LocalWords:  DialogueTree PatternDatabase
