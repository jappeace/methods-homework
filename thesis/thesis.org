...
\todo[inline]{Pershaps we can show trough experminetation what effect lower functions have?}
\todo[inline]{How many of the lower functions are used?}
\todo[inline]{Are slight differences in order relevant? [Ne, Ti] vs [Ti, Ne]}
\todo[inline]{I'm pretty sure current design doesn't work that great with 'deeper' levels, because we assumed irir or riri, but now the theory says riir or irri. We should analyze this difference I think}
...
trough connections.
To decide this we again take the example dialogue.
...
To confirm this, we implement a scenario such as the case study presented
in section [[Personality influence case study]].

\todo[inline]{Should probably be moved to appendix}
***** INTJ
...
3. We use the other scripted response (perhaps it thinks its ENFP),
...
**** Third test
After solving the issues from the second test we do anther run of all tests.
...
2. Said "of course" rather than give me some pain killers
   ...
   leading out of it.
   I also made give painkillers a low priority goal.
   Because currently there isn't really deep delibrartion going on.

**** Fourth test
Its the same day and we try another round to pass this relatively simple 
test.
...
*** Integration tests
**** Fix scenario issues
     1. Fix the remembered dialogue to store perlocutionary values.
        1. This is required for Fi
     2. Investigate why ENFP goes into the unexpected path.
        1. We may actually need to add before clauses to fix this properly.
     3. Do another test run.
