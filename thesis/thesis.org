...
\toReview
First of all are of course the chat bot systems. These are mostly reactive
systems that use pattern matching rules paired with good responses,
sometimes with conditions to allow for more variety.
An example of such a system is eliza bot which is described in
cite:galvao2004persona, where they also added personality with the OCEAN model.
...
Secondly a more precise an type allows a /pure function/ to do less different
things.
...
However Java can enforce it on a return type though.

\toReview
So if we assume we're dealing with pure functions,
we can reason about possible computations indirectly.
We will only start looking at what the function attitudes do precisely in
section [[Mapping to process]].
...
function, but according to A its just that one function.
Since we're assuming pure functions however B can only have read only extra  
information, however this is not enforceable in Java.
...
His reasoning domain was however in action space, but we still want to use the
idea that personality is a preference for a process rather than a preference for
content as discussed in section [[BDI + Personality]].
...
Information goes in, the function does its processing and then information goes
out. This is analogous to a mathematical pure function.
...
On the one hand for the functions, but on the other hand for the data used. \todo{wat?}
In our case this contains a believe base too.
...
an inquery based on "how is your disease?", or "why are you here?".
...
meaning,
but one meaning produces one string $\sigma$,
...
uttered.
We also need to mark which agent uttered, therefore we introduce $\Lambda$ as the set
of all agents, where $a \in \Lambda$.
...
#+RESULTS[b1afdd5626e9c343e4f16a7779dff1fe3d8acac2]: fig:dialoguetree
...
The meaning graph $G$ is a set of connections $(\pi, A, s_1, s_2)$
where $s_1, s_2 \in \mathcal{S} \wedge s_1 \neq s_2$, $A \subseteq \Lambda$ is the set of
...
Please note that this is probably not the best solution available,
It would have been better to use owl cite:world2012owl for example.
But this would open up a new problem on how to construct sentences,
of which the theoretical foundation is incomplete.
Another issue is how to interpret meanings, at where you could for example use
cite:moschitti2004study. But again its just a piece of the puzzle.
We think at this point making those steps are to big and probably simply will
result into failure.
...
personality in as a process. \todo{say this idea is similiar to information state transitions??}
...
When we looked at the work of campos we found the model the other agents as the
Feeling description. This describes $F_e$ rather well, $F_i$ not so much
however. So we adapted that for $F_e$ and for $F_i$ we used the lookup table.
...
Therefore $B = (h, [s], G)$, which looks like a relatively small believe base,
...
\todo[inline]{Figure out cardinality of drool subsystems, how many scoring sub systems how many scenario subsystems}
\todo[inline]{The case for a scenario subsystem per agent is the following: Each agent could have their own interpretation of the scenario}
\todo[inline]{Scoring system should almost defenitvly be 1}
\todo[inline]{We should describe a perfect architecture anyway, and then in the implementation we can describe an actual achitecture if there is a different one}
\todo{Note that splitting up the architecture is something we may do eventually once personality is implemented}
\cleared
This chapter is about how do we fit the algorithms discussed in chapter [[BDI and Jung]]
...
current implementation and the original architecture discussed in section [[
The serious game]].
...
#+RESULTS[6f973e79e5120972bc1a91a0e30be1e753fcb59a]: fig:architecture-concept
...
A meaning has connections to other meanings, which should be sensible responses.
...
We can do deliberation on this because we have one or more meaning /goals/.
...
** Substitute AIML bot 
\toReview
Because we are replacing a rather huge project on which quite a lot of legacy
code depends we don't want to just throw it away.
Instead we will be using a trick called a strategy pattern.
With this we first find a class that connects the chatbot to the rest of the
system.
In our case the best candidate for this was the Chat class that contains the
Chat session.
We bluntly replace this with an interface and let the Chat class extend that
interface (see section [[Minor issues]]).
The new interface is called ChatSession as can be seen in figure [[fig:
Now we can inject an alternative 'Chat', which will be a simple bot that
does the string to meaning conversion and inserts it into the drool system,
then it forks of a little worker that send the reply from the drools.

#+BEGIN_SRC plantuml :cache yes :file img/uml/class-substitute.png :exports results
  interface ChatBotEngine{
    +String chat(String request)
    +void setSession(Session session)
  }
 ...
 package nl.japppieklooster{
   package boterinterface{
     ChatBotFactory ..> ChatSession
     interface ChatBotFactory{
        +ChatSession create(String path, String name)
     }
     interface ChatSession{
      +HashMap<String, Object> getPredicates();
      +String multisentenceRespond(String str)
      +setKieSession(KieSession kie)
     }
   }
   package ymlbot{
     ChatBotFactory <|-- YapFactory 
     ChatSession <|-- YBot 
     class YapFactory
     class YBot
   }
...
#+END_SRC
#+CAPTION: Class diagram of the server where the aiml bot can be substitued by an alternative
#+LABEL: fig:class-substitute
#+RESULTS[b71504ec6f9fccdf79f1f076d51dfcf7041038e2]:
[[file:img/uml/class-substitute.png]]

\todo[inline]{New message processing activity diagram (see utterance processing)}
...
into type $S_t$ for meaning at time $t$.
...
#+RESULTS[30bcd9f9b7de22494e9d8c999d6325757162dceb]: fig:insert_meaning
...
<<static evaluation>>
\drafting
Static evaluation is a part of a heuristic.
Where the tree analyses is stopped and a guess is made about the value of a
node.

\drafting
We considered several methods of doing this.
The most obvious approach is of course just adding some kind of reward to
meaning uttered and let the path that leads to the most reward be the direction
we choose.
However we wish to avoid introducing new terms.

\drafting
To do this we considered using a technique called graph drawing cite:herman2000graph.
Where the meaning graph will be projected on a plane and where we can use
euclidean distance to guess how close we are to the goal.
The problem with this is approach is however that its compeletly dependent on
how the graph is drawn.
Which may misrepresent how closely a meaning is to a goal.

\drafting
Then we considered using information thoery as a basis.
So what is a simple good heuristic we could use? Well from a conversational
point of view a good meaning to be at is one that can lead to many meanings you
haven't encountered yet.
If one of those options leads to the goal we of course give it a huge bonus.

** Minor issues
<<Minor issues>>
\toReview
There was a problem in that the predicates attribute was public,
since an interface cannot have any attributes in Java, we decided to solve this
by replacing it by a method getPredicates.
In fact while doing this we quickly found out that there was no reason for this
attribute to be public in the first place
(common Java dogma requires the use of getters and setters).
* In conclusion
...
This is because it uses an utf8 encoding (see [[http://stackoverflow.com/questions/10748155/specified-key-was-too-long-max-key-length-is-767-bytes][this]]).
...
\todo[inline]{Extracting information from meanings isn't very DRY right now, for example saying how is your foot? Could perhaps be splitup into statusInquery + foot. However I don't know how to do this better right now, perhaps it could be a natural result of dubble matching but we lose the order inforamtion.}
** Tooling stuff
+ Protocol buffers could be used for protocol. Its strongly typed and has both
  C# and Java bindings
+ xml shouldn't be used as a procudual language (http://wiki.c2.com/?XmlAbuse)
...
** YML
**** Choose 
We could make symlink connections that link to multiple possible response.
file reason_here
#+BEGIN_SRC yml 
choose:
  + broken_arms
...
Then in the history of facts the reason_here would get the decison made by the
bot so reason_here(broken_arms) for example.
So every future reasone_here reference would also be the same reference.

We could even go as far as making this decision on load time.

**** Replace names by symbol values directly
Currently string are used for coupling, but we could replace this by the
symbol values directly.
I'm sure its possible for connections, less sure for things such as Choosing
**** History connections
...
** Rough planning.
1. Fix the bdi issues.
2. Finish implementing it
3. Once finished think about presenting it
   1. Show how it works with the discussed dialogue in background
   2. Perhaps implement mulitple scenarios
