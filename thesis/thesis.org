...
drools what's goin on. The injection of types will be treated in section [[Caviats]].
...
Tue connection is from the pattern to the template which is restricted to
the patient.

\toReview
There are also cases which simply mean to extend the patterns used:
...
\toReview
The srai tags simply say: use template for pattern x.
But we can translate that as just adding a regex to the "How long * pain" symbol.
