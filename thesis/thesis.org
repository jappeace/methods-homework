...
and some limited speech act theory [[Speech act thoery]].
...
[[personality theories]],
it is unclear on what theoretical foundation the claim was made this
...
and modelled formally. Similarly to the Fatima architecture there is no
personality research cited in this work.
However this formal model could be useful regardless of theory,
especially the observation that there are only three possible transformation
as the result of emotions (at least in their model): Transformation of actions
space, transformation of the utility function and transformation of probabilities
of state.
...
behavior. So if an agent would get several task the one he selects depends on
his personality values and the quality of execution would also depend on that.
