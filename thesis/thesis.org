...
$\mathcal{M}$ stands for the set of all encoded meanings[fn::Note that these
meanings are different than the ones found in the social practice.
A different word for the meaning we describe here is "symbol".] \todo{Replace meaning with symbol, to avoid confusion with social practice and the delete the footnote}
...
firstly we note that $\mathcal{M} \subset \mathcal{P}$,
...
#+BEGIN_SRC plantuml :cache yes :file img/uml/faprocessing.png :exports results
  start
  :interpret string into meaning (m);
...
#+RESULTS[503d2109720f17ed1daf7cb146e2b6b7ac65f207]: fig:faprocessing
...
\drafting
Both $F$ functions need a valuation function $h$: \todo{How is this represented in data?}
\[ m \overset{h}{\to} i \]
where $i \in \mathbb{N}$.
