...
Therefore we introduce the set of all possible speech acts as $\varrho$ of which 
a set of speech acts is $P \in \varrho$.
...
\todo[inline]{Perhaps we should move this into function ply by depth and just refer to that from here}
\toReview
...
Doing this approach will make a rational function that is first do nothing,
...
However it didn't treat another issue in dept, which is how does a function know
at which level it should operate.
We just assumed that rational should always act 1 level above the leaf nodes,
and irrational at the leaf nodes.
This would make a rational function in the first position the least
relevant function, since in the first pass it does nothing and when everything
...
Then we can also make a decision about whether a function should operate at a
particular height, or go downward trough the entire tree, we chose to let it go
downward because then the first rationale function would be most pronounced.
...
\toReview
To calculate the operation height, we need to know the function order,
...
For the precise implementation we refer the reader to the code, since
its rather complex. \ask{Can I do this?}
