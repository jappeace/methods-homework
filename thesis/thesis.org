...
Herein we will discuss some of the bigger chalanges of implementing the bot,
however as the accaptance tests show the bot is functional.
We first start of with analyzing AIML and clarify our thought process on
replacing it with another dataformat.
Then we will go trough a scenario step by step and talk about how the bot does
some of the thing its supposed to do.
Finally we will present the test that indicate that we indeed succeeded.

\toReview
However, when one starts working on an existing software project the first
obstacle faced is building/executing the project,
this is discussed in detail in the appendix [[building]].
