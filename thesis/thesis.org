...
** TODO A type signature approach
\cleared
...
$\mathcal{S}$ stands for the set of all encoded symbols[fn::Orignally this was
called meaning with an $m$, but we want to avoid confusion with meaning in the
social practice, and therefore renamed it to symbol, as in symbolic representation] \todo{Replace meaning with symbol, to avoid confusion with social practice and the delete the footnote}
...
meaning, but one meaning always produces one string $\sigma$. \todo{Can't you do random selection on the result?}
...
\[ B_t \to t \to S_t \overset{f_a}{\to} (B_{t+1}, A_{t+1}) \]
...
\[ B_t \to t \to S_t \overset{f_a}{\to} (B_{t+1}, R_{t+1}) \]
\todo[inline]{Perhaps we need additional transition between R -> D, perhaps the entire R step can be left out even and do A -> D}
...
\[ D = (s, [D])\]
\todo{I think this misses the agent who said the meaning}
...
The most preferred reply is the first element in the list of children. \todo{Perhaps we can use an object diagram to make the treelike nature more clear}
...
#+RESULTS[00269a7ff59de81c42c843e550215a0acf983ea0]: fig:faprocessing
...
#+RESULTS[229649447467bceedd0388ab3f03bc220fb0f24b]: fig:architecture-concept
