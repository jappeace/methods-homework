...
#+LATEX_HEADER: \newcommand{\newlyCleared}{\todo[noline, backgroundcolor=white, bordercolor=red]{Newly doubleCleared}}
...
*** Function ply depth
 ...
 doing a pure /next/ based appraoch and we re-introduced the list mechanism 
 ...
 Partly because we have a hybred approach of deciding the /next/ function,
 and because we simply hadn't worked it out for the pure /next/ based approach
 we need to anwser the follwoing question:
...
in one package.
These concepts have now been split,
the file reader handles the loading part of the bot,
after that it just insert the pattern,
symbol and connection database into the drools rule engine.
...
it should be really easy to support other data formats.
...
but it is basically to serperate the system from the agent believes.
...
Also note that we introduced the social practice component as a way for the bots
to predict how the scenario will go.
The score component has direct acces to the bot facts to evaluate how the user
...
** Rough planning.
*** Report
+ Clear the undoubleCleared sections: `Everything newly doubleCleared'
