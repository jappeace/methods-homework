...
\toReview
Se we have some believes, time and meaning going in, some deliberation
...
\toReview
Therefore we introduce $D$ a dialogue tree immutable data structure:
\[ D = (m, [D])\]
Where $m \in \mathcal{M}$ for the meaning,
and $[D]$ is the ordered list of children.
...
and then we put out a dialogue tree plus the replies.
...
way would make it impossible for function attitudes to inspect resuls
...
\todo[inline]{This may work for irrational, but how to do this on rational? We can't do depth first ordering on the tree it would make no sense. Perhaps we can do /e/ as just ordering the my own preferences wheras i goes to the predicted other's prefernces?}
...
(see section [[Extraversion introversion]]),
