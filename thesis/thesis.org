...
** Personality theories
   ...
   reasonable consistency cite:mischel2008introductionp3_definition.
   ...
   but for psychology it is a good definition. Since not a program has to work
   with it but a human, who can determine for himself what these things are.
...
*** The big five
 ...
 others. Catell used a rating scheme, where a trait was introduced and
 everyone then had to rate everyone else as average, below or above average.
 Persons could also use one extreme rating per trait for one person.
 These traits where based on the /personality sphere/ concept which
 tried to cover the entire surface of personality by providing many small trait
 areas. Examples of the traits are: "Attention getting vs Self sufficient", or
 ...
 Since each of them decide to reduce the OCEAN model in some kind of way
 we can conclude that although OCEAN is good for discussing the psyche,
 but it is either incomplete or inssuficuent for a software sepecitifaction role. 
 
*** Personality types
\todo[inline]{Give some examples where we expect personality to be different per model? This then can be used as test casus for the eventual bot}
 <<sec:types>>
 \cleared
...
<<Jungian types>>
 ...
 and therefore be more like a scientist, where he would devote much of his time
 on studying natural phenomena, for example a biology researcher would study
 natural objects and behaviors and is therefore more likely to have extroverted
 thinking as dominant function.
 ...
 consistency of the internal reasoning process is most important.
 ...
 + $S_e$ stands for extroverted sensing, Act on concrete data from the here and
   now. Then lets it go.
   People of this type are often realistic and practical. A typical
   profession driver of heavy machinery or athlete cite:seproffesionadvice, 
   because living in the moment is most important for those professions which
   comes natural to $S_e$.
 + $S_i$ stands for introverted sensing, acts on concrete data from memories and
   ...
   A possible profession for the people with $S_i$ as dominant function is in
    quality assurance, where the perfect model in their mind can be easily
 ...
 + $N_e$ stands for extraverted intuition, try to find possibilities in every
   ...
   almost every situation, they also makes them very inspiring leaders because
...
**** MBTI
 ...
 \toReview
 MBTI has always produced a continues scoring system.
 ...
 the case and a continues scale for assessment judgements is required.

 \toReview
 In cite:sipps1985item it is put trough a method called factor analyses.
 This is similiar to the technique where OCEAN is based upon (see section [[OCEAN]]).
 ...
 cite:sipps1985item explains the first extra factor as "unconditional positive",
...
**** PPSDQ
 ...
 sample in which the proto PPSDQ was submitted and the MBTI itself, factor
 analyses was used to determine correlation, this is done in
 cite:thompson1994concurrent.
 The optimal amount of points presented is a subject for debate.
 Common sense would suggest that more points would give more precision,
 however in cite:matell1971there it is suggested that reliability and validity
 do not increase with more points. In cite:garland1991mid however they
 state the importance of an available midpoint.
 The 5 points in the PPSDQ are not motivated.
 Note that the amount of available options presented in these kind of
 test give an inherent bias to the results. If there are 3 choices for
 example you may pick more extreme answers than a 5 option choice test, since with
 five options you can go less extreme but still in a direction. \todo{Cite this?}
...
#+NAME: tab:word-pair-example
 ...
 \todo[inline]{How are these word pairs obtained?}
...
**** TODO Concrete implementations
 ...
 \todo{Again perhaps I should consider deleting this?}
 \todo[inline]{2apl}
...
** The serious game
 <<The serious game>>
\todo[inline]{this section is not complete because I haven't gotten the game to work propperly I think (or the game is incomplete which mean I describe it way better than that it actually is)}
  \cleared
...
\todo[inline]{add archtitecture image}
    \cleared
...
**** Text processing
    \cleared
     ...
     \todo{chatbot is just a fall trough mechanism and the drools are central}
     This bot does the parsing and validation of AIML,
     after this is done it specifies a response
     based on the AIML.
     The ALICE bot is used as a library by the wildfly application that
     handles web socket comminucation and game tracking.
 cite:augello2016social
**** Deliberation
    \cleared
     To do deliberation ALICE was changed to allow for extra tags.
     These are hooks to determine when specific drools should be used.
     As of right now the only thing the drools do is modify the emotion of the
     agent which is reflected in the scenario score.
     It appears as if the entire bot is only reactive.
     All deliberation is done in the predefined xml files which is quite limited
     in scope.
     The only "extra" deliberation happens in the drools, which as of right now
     only handles emotions.

\todo[inline]{Drools is the bdi basis but it still lacks the bdi /structure/}
...
We will reduce the scope of the deliberation function by using a haskell like
syntax (or category theory cite:spivak2013category).
\todo{Either replace category theory with haskell like syntax or learn it and cite it}
Then we will also look at how to convert user utterances to objects we can do
reasoning with.
...
\drafting
test uml
#+BEGIN_SRC plantuml :file img/uml/sequence1.png :exports results
  ' this is a comment
