...
We worked arround this Issue by introducing a structure that contained the
...
and although we can do this in Java with annonymous classes, 
...
This actually also introduces an infinite creation sequence,
...
Therefore the =JungFuncAccessor= inteface was introduced.
This allows unit test to check if the next function was called for exampple,
but the code also generally becomes more extendable because of this.
...
progress. Its relativly easy to add other rules that can modify the process,
