...
\drafting
There are several phases in the bot currently
...
Since the notation may be complete foreign to the interested reader at this
point an activity diagram can be seen in figure [[fig:faprocessing]].
...
<<Rational and irrational>>
\cleared
Up till now we modeled the type signature to have a dialogue tree as input and
output.
...
** Mapping function attitudes to a process 
   \todo[inline]{Entire BDI process, every thinking stap should be analyzed and see how personality could have influenced it. Then give arguments why we do it in a particular way. So owl should for example also be considered for knowledge representation}
<<Mapping to process>>
\cleared
