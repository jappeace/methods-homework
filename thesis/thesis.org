...
\drafting
To find the height, we take the list $[F_a]$ and zip it with list $[F_a]'$, 
which is $[F_a]$ but with the last element as the first and all other 
elements shifted one deeper.
For example if we have:
\[[F_a] = [T_i, S_e, N_e, F_e] \]
Then:
\[[F_a]' = [F_e, T_i, S_e, N_e] \]
With the zipped result being:
\[zip [F_a] [F_a]' = [(T_i, F_e), (S_e, T_i), (N_e, S_e), (F_e, N_e)] \]
