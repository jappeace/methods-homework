...
It is based upon folk psycology, ie common sense is used.
...
Intentions are plans to make a desire come to fruition.
...
verification and construction cite:rao1995bdi.
If these things are true is another matter.

*** logic of BDI
\todo[inline]{cohen \& levesque}
\todo[inline]{Rao and georgeff}
\todo[inline]{karo \& jj}
\todo[inline]{Agotness cause it crazy}
...
underlying jungian functions, whereas in campos the measured dimensions were
used.
Translating to the functions has some advantages,
by doing so we are for example not bound to just the MBTI.
We also get more accurate descriptions of what Jungian functions are, Jung
described in his work people with that particular function as dominant.
But this is harder to do with the dimensions, because if you take an INTJ and an
...
Therefore we chose to instead simplify types to orders of Jungian function
attitudes.
...
Since Jung wasn't much of a mathematician cite:jungonfunctionsn its just an
informal definition.
...
between the functions and define a prototype for the individual functions.
...
following pure function prototype[fn::Using a Haskell like syntax where the
arrows indicate functions, left of the arrow is the argument and right is the
result, alternativly this is similar to category theory] \todo{cite the haskell and category theory?}
...
Then we can define $M_t \subset \mathcal{M}$ which stands for the meaning we matched on
at time $t$.
...
This produces the following prototype:
...
however, it is a lot harder to constrain this formally.
For example when the agent is asked to sit on the counter,
how should this be encoded,
if we constrained the agent with just answering questions?
Movement should be possible for example but movement like one does during sky
diving is not interesting.
Therefore we constrain the set of possible actions informally with help of
the social practice (see [[social practice]]).

** Composing type dynamics
\toReview
The first thing an commendable programmer may think of when trying to combine
...
Since the prototype of the $f_a$ function does not have a similar type of
...
\[ \left (\overset{next}{B_t \to t \to M_t \to (B_{t+1}, A_{t+1})}\right ) \to B_t \to t \to M_t \overset{f_a}{\to} (B_{t+1}, A_{t+1}) \]
...
As unit a function the action that results in a message: "I don't know".
By unit function we mean the initial /next/ function.

\toReview
To illustrate this prototype design more clearly we'll sketch an example
with the first two functions of the INTJ type:
\[N_i > T_e \]
So to encode this as a function we start with the least preferred type namely
the $T_e$, however to let it play an advisory role in the $N_i$ function we first
need to complete the /next/ argument.
Because its the least preferred function we just use the unit.
Now the curried type of $T_e$ satisfies that of $N_i$ and we can use it as
/next/.
