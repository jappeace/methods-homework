...
There are two ways of circumventing this.
...
always have setters.
So we can just genarete some java code for that.

\toReview
Finally the last thing that S-AIML introduced were scenes.
We already have a way of representing these trough folders.
...
* Conversion script
The conversion script is located in the root of the git project, called
`conversion.py`.
...
Run this script on top of the desired S-AIML project.
Then rebuild the YAML capable bot, don't forget to update scenario name
in ChatBotEngineImpl of the server.
...
** Rough planning.
*** Prepare a demo
   + Fix the double reply bug, this turned out to just be a double match which we want
   + Do the integration test once again
   + Test it in Unity!
   + Make a 'script' for giving the demo
   + Print the thesis?
*** Report
