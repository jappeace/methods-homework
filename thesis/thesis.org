...
In this they move from personality values in OCEAN towards action selection
fuzzy logic is used.
Fuzzy logic `obfusicates' `crisp' values with more natural terms,
rather than ${1,2,3}$, we can use low, medium, and high.
With these terms, buisness rules can be specified.
...
``if actions is high or selfconsciousness is high then acceptance is favored''.
...
Modulirazation which is partly already done in the paper by splitting up action
selection and action revision for example
(although we disagree with splitting up the facets along these lines).
...
\dignum{Tell more about this, what means fine grained, how does the architecture of campos look?}
