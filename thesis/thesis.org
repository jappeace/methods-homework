...
The innovation presented there was the separation from task knowledge of
dialogue knowledge.
...
*** Symbol to string
\toReview
So the next step is to consider how we go from symbols back to strings.
What we assume is that the agent already found a symbol to utter, for 
example the status inquiry.
...
because the name template is very generic but they function the same:
...
\toReview
We can make this syntax even more terse.
Observe how one category always will have one symbol.
...
If TOML were to stabilize, its probably better to move to that instead.
Currently the best choice is YAML,
do note that its relatively easy to move between these standards,
...
\toReview
We keep the pattern tags for legacy support.
However its much more easy to just support regular expresions
cite:thompson1968programming.
The reason for this is that they're part of the Java standard library
cite:regexpattern.
What we can do is transform AIML patterns into regular expresions,
...
*** Core idea
\toReview
Once we split up the template from the patterns,
...
In listing [[code:yaml:simple]] we connected the symbols discussed in our orignal
...
#+RESULTS[16e838a3d596cccc9d290e17208b8fb00ecb9ef1]: fig:dep:connections
...
\toReview
In listing [[code:yaml:complex]] we can quickly see in which direction this
conversation goes once from is matched.
From ask reason here to need medicine, from need medicine to why need etc.
...
** Supporting more exotic features
\drafting
AIML is a big standard and has various useful features that need to be
supported for a competing standard to make any chance.
We also require to support several features from S-AIML to make social practice
integration more easy in the future.
...
*** Insert tags
\drafting
Insert tags are no longer neccisary because symbols get always inserted.
We can simply make events happen if a particular event happens by matching a
symbol on name. \todo{is this true?}
*** Drool tags
\drafting
There is no real mechanism for emulating the behavior of drool tags,
except for the fact that symbols that have templates can be manually
filled up by constructing a =MatchedQueryDB= manually in drools and filling it
up with your own content.
