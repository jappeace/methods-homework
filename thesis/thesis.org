...
\todo[inline]{Maybe a package overview subsection, where we discuss the packages and their goals}
*** Data structures
...
With these symbols the CaputerMatchDB is stored, which is later used to make an
utterance from.
This isn't done imediatly because believes are required to create the
uttereance.
We for example need to know which connection was walked to get here to figure
out the =PerlocationaryValueSet=.
...
\toReview
By studying figure [[fig:supportclassdrools]] we can start to undrestand what is
going on inside the drools.
We can for example see that a distinction is made for when a result matches
an inscene pattern or a neigbouring scene pattern.
...
This is of course the result of the template match searched in the utterance
history.
...
We manage this inside drools to give other rules the oppertunity to inspect
...
*** Initialization
\toReview
Since we are dealing with a potentially cyclic datastructure,
which we also want to be immutable, we have a difficult problem.
A potential way of working around such a problem is having a mutable,
...
\toReview
This way of doing things is the reason why we chose the order of initialization
shown in figure [[fig:initjung]].
The cyclic structure we want to create is the symbol graph $G$.
So we start with the non-cyclic nodes, the symbols by constructing the symbol
database.
All symbols area constructed and put into the symbol database with as key the
scene and name value.
...
\toReview
Finally the patern databases are constructed.
...
group them by scene.
For neigbouring scene patterns however connections are required.
The reason for postponing these to the end of construction is because the
neigbouring scene patern database requires the knowledge of connections.
...
*** Operation
 \drafting
 To understand how the opperation of a bot works we can look at it best from the
 point when a message is received and walk trough the steps it takes.
 An overview of this process is givin in the figure [[fig:insert_meaning]].
...
**** Prepare script
***** Template stuff
      In Yaml, the regex and the templating
      Then run it and show it works
***** Before tags
      In Yaml show where you did that
      Then run it and show it works
***** Scene transitions
      Just go to the next scene and show intro stuff doesn't work anymore
**** Email
**** Print
*** Report
