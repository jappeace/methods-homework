...
**** Text processing
    \toReview
     ...
     parsing and validation of AIML, after this is done it specifies a response
     based on the AIML.
...
**** Deliberation
    ...
    \todo[inline]{drools, Lucas didn't write about them...} 
* BDI and Jung
<<Jungian BDI>>
\toReview
...
We will reduce the scope off the personality function by using category theory
cite:spivak2013category.
\todo{Either replace category theory with haskell like syntax or learn it and cite it}
Then we will also look at how to convert user utterances to game theoretic
actions.
...
After this we will look what the categories that were introduced mean, in an
attempt to reduce the amount of believes.
...
function attitudes.
This is done with a Haskell like syntax (or category theory), \todo{choose}
...
definition possible definition towards one that fits the project scope.
...
in which a process is preferred. So $f_a$ can be any of the function attitudes.
...
so firstly we note that $\mathcal{M} \subset \mathcal{S}$,
...
To ensure a reactive and proactive agent we also have to pass the current time
as argument.
...
\todo{I'm thinking of moddeling the meaning input as a tree, and the output as a tree. With the left most child of the rootnode being the most preffered output.}
\drafting
An important observation to note is that we used a set rather than an element
for a meaning.
This is intentional.
There are several reasons for this,
we do not want to exclude the possibility of several meanings at time $t$.
Secondly to deliberate if a possible response is adequate we need to be able
to test related meanings into a game tree.
So for example if an $f_a$ wants to know what the other will reply
to a particular meaning $m$, he can simply pass that meaning to another $f_a$
and look at the response.
\todo[inline]{This meaning is incomplete, it should be a game tree from which meaning to meaning makes sense. IE according to the perceiving functions their data structures, every level on the tree should be someone saying something. The root node being the user utterence if reactive and the chatbot if proactive. Then the judgement functions can figure out how close we are getting towards the goal}
...
meanings as children. \todo{I should change meaning input to a tree structure}
...
\drafting
We can model extroversion and introversion as a preference for using a depth first
or breadth first search in the data structures used to either make a decision or
generate possibilities.
So for example when an extroverted irrational function wants to generate
possibilities, it will look at its data source (meaning graphs that are either
ideas or objects) and then look at each possibility around the current one to
make a reply. 

\todo[inline]{alternativly I just may drop this...}
...
What (initial) believes
as are necessary to accomplish this.
The $F_i$ function needs its personal value structure for example.
...
| $S_e$    | Information from the senses                       |
| $S_i$    | Memories from the senses                          |
| $N_e$    | A situation graph to jump to?                     |
| $N_i$    | An idea graph?                                    |

\todo[inline]{what is N? Right now I'm thinking the difference between S and N is just two different graphs of connected meanings, Or perhaps N uses a meaning graph and S uses sensory information somehow?}
...
** TODO Encoding goals
   Judgment is mainly about figuring out which actions are going into the right
   direction of a goal.
   \todo[inline]{Can we leave goals implicit to the social context?}
